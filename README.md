...
현재 저장소는 로컬 Google 로그인, YouTube OAuth, 공개 영상 댓글 수집,
Supabase 저장, OpenAI 분류와 Comment Inbox까지 첫 번째 실제 흐름을
검증할 수 있는 단계입니다.
...
```text
feature/{작업자 이름}/{기능 이름}
```

- 작업자 이름과 기능 이름은 영문 소문자로 작성합니다.
- 기능 이름의 단어 구분에는 밑줄(`_`)을 사용합니다.
- `codex/`, `feat/`처럼 도구나 축약어를 앞에 붙이지 않습니다.
- 예시: `feature/junyoung/comment_inbox_ui`
