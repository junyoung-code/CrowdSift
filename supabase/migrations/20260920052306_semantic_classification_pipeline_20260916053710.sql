-- Additive semantic pipeline. Historical outputs and source records remain intact.
alter table public.analysis_jobs add column execution_config jsonb;
alter table public.analysis_jobs add column replacement_job_id uuid references public.analysis_jobs(id);
alter table public.classification_verdicts add column pipeline_version text not null default 'legacy';
alter table public.classification_verdicts add column semantic_trace jsonb;
alter table public.classification_verdicts add column rewrite_status text not null default 'not_required'
  check (rewrite_status in ('pending', 'accepted', 'failed', 'not_required'));
alter table public.classification_feedback add column correction_reason text;
alter table public.classification_feedback add column application_context text;

alter table public.analysis_job_items add constraint analysis_items_identity_key unique (id, workspace_id, raw_comment_id);
create table public.semantic_snapshots (
  analysis_job_item_id uuid primary key,
  ...
  unique (analysis_job_item_id, workspace_id),
  foreign key (analysis_job_item_id, workspace_id, raw_comment_id)
    references public.analysis_job_items(id, workspace_id, raw_comment_id) on delete cascade
);
create table public.semantic_attempts (
  id uuid primary key default gen_random_uuid(),
  analysis_job_item_id uuid not null,
  workspace_id uuid not null,
  stage text not null check (stage in ('semantic_analysis','feedback_rewrite','rewrite_validation')),
  ...
  error_code text,
  created_at timestamptz not null default now(),
  unique (analysis_job_item_id, stage, attempt),
  foreign key (analysis_job_item_id, workspace_id)
    references public.semantic_snapshots(analysis_job_item_id, workspace_id) on delete cascade
);
create index semantic_snapshots_workspace on public.semantic_snapshots(workspace_id, raw_comment_id);
create index semantic_attempts_workspace on public.semantic_attempts(workspace_id, analysis_job_item_id);
alter table public.semantic_snapshots enable row level security;
alter table public.semantic_attempts enable row level security;
create policy "members read semantic snapshots" on public.semantic_snapshots for select to authenticated using (public.is_workspace_member(workspace_id));
create policy "members read semantic attempts" on public.semantic_attempts for select to authenticated using (public.is_workspace_member(workspace_id));
revoke all on public.semantic_snapshots, public.semantic_attempts from public, anon, authenticated;
grant select on public.semantic_snapshots, public.semantic_attempts to authenticated;
grant all on public.semantic_snapshots, public.semantic_attempts to service_role;

-- Incomplete legacy work moves to new item IDs. No legacy stage can be resumed as semantic data.
create function public.supersede_legacy_classification_job(target_job_id uuid, new_configuration_key text, new_execution_config jsonb)
...
begin
  ...
  on conflict (import_job_id,configuration_key) do update set configuration_key=excluded.configuration_key
...
-- Explicitly retry only the rewrite cycle; preserve analysis, verdict, and prior attempts.
create function public.retry_semantic_rewrite(target_verdict_id uuid, target_workspace_id uuid)
...
begin
  ...
  -- A boundary marker lets the next cycle keep earlier attempts for audit.
  ...
  insert into public.audit_logs(workspace_id,event_type,target_type,target_id,metadata)
  values(target_workspace_id,'classification.rewrite_retry','analysis_job_item',item_id,jsonb_build_object('afterAttemptId',(select id from public.semantic_attempts where analysis_job_item_id=item_id order by created_at desc,id desc limit 1)));
end $$;
...
create or replace function public.get_inbox_feed_page(
  target_workspace_id uuid,
  review_levels public.review_level[] default array['safe'::public.review_level, 'caution'::public.review_level, 'risk'::public.review_level],
  ...
  classification_status_filter text default null,
...
begin
  ...
  if classification_status_filter is not null and classification_status_filter not in ('decided', 'review_queue') then
    raise exception 'invalid classification status' using errcode = '22023';
  end if;
  if period_filter is null or period_filter not in ('all', '7d', '30d', '90d') then
    raise exception 'invalid period' using errcode = '22023';
  end if;
  if sort_order is null or sort_order not in ('latest', 'likes') then
  ...
  reply_rows as (
    select
      ...
      coalesce(reply_rewrite.rewritten, case when reply_verdict.pipeline_version <> 'semantic-v1' then reply_verdict.feedback_core end)
        as neutral_text,
    ...
    left join lateral (
      select cv.level, cv.hide_source, cv.feedback_core, cv.pipeline_version
  ...
  inbox_rows as (
    select
      ...
      case when cv.pipeline_version='semantic-v1' then (cv.semantic_trace->>'confidence')::real else null::real end as confidence,
      coalesce(
      ...
      coalesce(
        ...
        case when cv.pipeline_version <> 'semantic-v1' then cv.feedback_core end
      ) as neutral_text,
      null::text as normalized_question,
      case
  ...
  where (
    ...
    -- Apply all conditions before count/order/pagination. A held-only query
    -- must not be limited to the first hundred comments in the workspace.
    and (
      classification_status_filter = 'review_queue'
      or review_levels is null
      or review_levels @> array['safe', 'caution', 'risk']::public.review_level[]
      or ir.review_level = any(review_levels)
    )
    and (
      classification_status_filter is null
      or (classification_status_filter = 'review_queue'
        and ir.classification_status = 'review_queue' and ir.review_level is null)
      or (classification_status_filter = 'decided'
        and (ir.classification_status = 'decided' or ir.review_level is not null))
    )
    and (
      period_filter = 'all'
      or ir.published_at >= (
        ((current_timestamp at time zone 'Asia/Seoul')::date
          - case period_filter when '7d' then 6 when '30d' then 29 else 89 end)::timestamp
    ...
    and (
      ...
      or coalesce(ir.author_display_name, '') ilike '%' || search_query || '%'
    )
  order by
    case when sort_order = 'likes' then ir.like_count end desc nulls last,
...
revoke all on function public.get_inbox_feed_page(
  uuid, public.review_level[], public.comment_category, text[], text,
  public.action_state, text, real, real, integer, integer, text, text, text
) from public, anon;
grant execute on function public.get_inbox_feed_page(
  uuid, public.review_level[], public.comment_category, text[], text,
  public.action_state, text, real, real, integer, integer, text, text, text
) to authenticated;
