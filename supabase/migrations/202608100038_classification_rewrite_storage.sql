-- 순화문은 원문을 대신하지 않는다. Terra 가 뽑은 피드백 핵심과도 분리해
-- 별도 행으로 남기고, 화면에는 검사를 통과한 것만 내보낸다.

alter table public.classification_stage_runs
  drop constraint classification_stage_runs_stage_check;

alter table public.classification_stage_runs
  add constraint classification_stage_runs_stage_check
  check (stage in ('moderation', 'luna', 'terra', 'rewrite'));

create table public.classification_rewrites (
  ...
  classification_verdict_id uuid not null
    references public.classification_verdicts(id) on delete cascade,
  rewritten text not null check (length(trim(rewritten)) > 0),
  tone_variant text not null
    check (tone_variant in ('neutral', 'friendly', 'soft_disappointment')),
  -- 검사에서 걸린 순화문도 남긴다. 화면에는 나가지 않지만 왜 걸렸는지는 봐야 한다.
  accepted boolean not null,
  rejections jsonb not null default '[]',
  created_at timestamptz not null default now()
);

create index classification_rewrites_workspace_comment_idx
  on public.classification_rewrites(workspace_id, raw_comment_id, created_at desc);

create index classification_rewrites_verdict_idx
  on public.classification_rewrites(classification_verdict_id);

alter table public.classification_rewrites enable row level security;

create policy "members read classification rewrites"
  on public.classification_rewrites for select
  using (public.is_workspace_member(workspace_id));

revoke all on table public.classification_rewrites
from public, anon, authenticated;

grant select on table public.classification_rewrites to authenticated;

grant select, insert, update, delete on table public.classification_rewrites
to service_role;

-- Inbox 가 읽는 한 칸에 순화문을 먼저 내보낸다.
-- 사용자가 직접 고친 문장이 있으면 그것이 여전히 가장 앞선다.
...
CREATE OR REPLACE FUNCTION public.get_inbox_conversation_page(target_workspace_id uuid, review_levels review_level[] DEFAULT ARRAY['caution'::review_level, 'risk'::review_level], category_filter comment_category DEFAULT NULL::comment_category, video_id text DEFAULT NULL::text, analysis_state_filter text DEFAULT NULL::text, action_state_filter action_state DEFAULT NULL::action_state, search_query text DEFAULT NULL::text, min_confidence real DEFAULT NULL::real, max_confidence real DEFAULT NULL::real, page_size integer DEFAULT 25, page_offset integer DEFAULT 0)
 RETURNS TABLE(raw_comment_id uuid, source_import_job_id uuid, source_kind comment_source_kind, youtube_video_id text, video_title text, video_thumbnail_url text, author_display_name text, author_avatar_url text, published_at timestamp with time zone, like_count integer, source_available boolean, safe_source_text text, analysis_id uuid, classification_status text, classification_trace jsonb, category comment_category, review_level review_level, confidence real, recommended_action recommended_action, manual_review boolean, neutral_text text, normalized_question text, analysis_state text, action_state action_state, delete_eligible boolean, reply_count bigint, replies jsonb, total_count bigint)
 LANGUAGE plpgsql
