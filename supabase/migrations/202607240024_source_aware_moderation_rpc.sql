...
begin
  ...
  if not exists (
    ...
    where cii.import_job_id = target_source_import_job_id
      and cii.workspace_id = target_workspace_id
      and cii.raw_comment_id = target_raw_comment_id
      and cij.source_kind = 'owned_oauth'
  ) then
    raise exception 'moderation source observation is not owned'
      using errcode = '42501';
  end if;
  ...
  if created_request_id is null then
    ...
    where mar.idempotency_key = target_idempotency_key
      ...
      and mar.source_import_job_id = target_source_import_job_id
