-- Inbox 에서 영상을 여럿 골라 볼 수 있게 한다.
--
-- 한 번에 하나만 고를 수 있어서, 영상 둘에 나눠 단 묶음을 함께 보려면 필터를
-- 바꿔 가며 두 번 읽어야 했다. 판단이 영상마다 어떻게 갈리는지는 나란히 놓고
-- 봐야 보인다.
--
-- 빈 배열은 「고르지 않음」으로 읽어 전체를 보여 준다. null 과 같게 두지 않으면
-- 사용자가 체크를 모두 해제했을 때 결과가 0건이 되어 고장처럼 보인다.
