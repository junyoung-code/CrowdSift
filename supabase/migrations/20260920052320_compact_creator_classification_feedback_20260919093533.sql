-- Keep a creator's hold separate from the three concrete review levels.
alter table public.classification_feedback
  add column corrected_status text
  check (corrected_status in ('decided', 'review_queue'));

alter table public.classification_feedback
  drop constraint classification_feedback_check;

alter table public.classification_feedback
  add constraint classification_feedback_corrected_outcome_check
  check (
    decision <> 'corrected'
    or (
      coalesce(corrected_status, 'decided') = 'decided'
      and corrected_level is not null
    )
    or (
      corrected_status = 'review_queue'
      and corrected_level is null
    )
  ),
  add constraint classification_feedback_hold_not_personalized_check
  check (
    corrected_status is distinct from 'review_queue'
    or not use_for_personalization
  );
  ...
  inbox_rows as (
    select
      ...
      case
        when (
          case
            when latest_feedback.corrected_status = 'review_queue' then null
            else coalesce(latest_feedback.corrected_level, cv.level)
          end
      ...
      case
        when latest_feedback.corrected_status is not null
          then latest_feedback.corrected_status
        when latest_feedback.corrected_level is not null
          then 'decided'
        else cv.status
