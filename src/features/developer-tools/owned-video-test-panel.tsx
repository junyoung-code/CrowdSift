...
} from "@/app/(product)/app/videos/actions";
...
export async function OwnedVideoTestPanel({
  parameters,
  workspaceId,
}: OwnedVideoTestPanelProps) {
  ...
  if (connectionError || channelError || videosError || jobsError) {
  ...
  return (
    <section className="videos-page" aria-labelledby="owned-video-test-title">
      <div className="page-heading video-page-heading">
        <div>
          <p>OWNED CHANNEL TEST</p>
          <h2 id="owned-video-test-title">내 채널 영상으로 테스트</h2>
          <span>
