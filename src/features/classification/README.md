...
- **거친 비판 8건** — 욕설 없이 거칠게 비판하는 댓글이 안전으로 빠졌다. 기준 문서 세 곳의 요약이 서로 달랐던 것이 원인이며 프롬프트 v5/v3에서 고쳤다. 재측정 결과는 로드맵 10번에 붙는다.
- **검토 대기 9~11건** — 대부분 답글이다. 부모 댓글이 없으면 원리적으로 판단할 수 없다. 답글 맥락 기능이 미결이다.
