...
- **회차마다 흔들리는 판정이 있다** — `노잼이다 진짜`, `알려주면 큰일 나냐` 는 하루 동안 주의와 안전을 여러 번 오갔다. 프롬프트로 고칠 문제가 아니라 판정이 경계에 걸쳐 있다는 뜻이다. 확인하려면 같은 프롬프트로 여러 번 돌려 평균을 봐야 하는데, 지금 측정은 회차당 한 번이라 가릴 수 없다.
- **스레드는 한 단계만 본다** — 부모의 부모는 넘기지 않는다. 계획서의 답글 미결 7개 중 등급 전이·스레드 전체 숨김 같은 화면 쪽 규칙은 아직 정하지 않았다.
- **위험은 한 방향 문이다** — Luna가 잘못 올린 위험을 Terra가 되돌릴 수 없다. `개노답이다`가 그 사례였는데, 부모 원문을 넘기자 Luna가 애초에 위험으로 올리지 않아 다른 층위에서 풀렸다. 규칙 자체는 그대로다.
