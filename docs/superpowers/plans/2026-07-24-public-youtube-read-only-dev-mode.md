...
- [ ] **Step 1: Next.js 16 환경변수와 server/client 경계 문서를 끝까지 읽는다**
...
- [ ] **Step 3: 최소 환경 gate와 parser를 구현한다**
...
- [ ] **Step 4: 환경 문서와 server schema를 갱신한다**
...
- [ ] **Step 5: 테스트를 통과시키고 Task 범위를 확인한다**
...
- [ ] **Step 1: Google provider mapping과 오류의 실패 테스트를 작성한다**
...
- [ ] **Step 2: application-owned read contract를 만든다**
...
- [ ] **Step 3: API Key 기반 adapter와 factory를 구현한다**
...
- [ ] **Step 4: preview service에서 auth 이후 gate와 URL을 재검증한다**
...
- [ ] **Step 1: 현재 schema와 Supabase test 패턴을 확인한다**
...
- [ ] **Step 3: source provenance migration을 구현한다**
...
- [ ] **Step 4: 비용 snapshot table과 RLS를 구현한다**
...
- [ ] **Step 5: source-aware DB guard를 구현한다**
...
- [ ] **Step 6: DB reset/test와 타입 생성을 실행한다**
...
- [ ] **Step 1: 총 수와 부모 무결성의 실패 테스트를 작성한다**
...
- [ ] **Step 1: 공개 job 계약과 dispatch의 실패 테스트를 작성한다**
...
- [ ] **Step 2: public job creation service를 구현한다**
...
- [ ] **Step 3: process dispatcher와 persistence를 구현한다**
...
- [ ] **Step 4: persisted status endpoint를 구현한다**
...
- [ ] **Step 1: routing table의 실패 테스트를 작성한다**
...
- [ ] **Step 2: code-owned routing을 구현하고 RAG 시점을 옮긴다**
...
- [ ] **Step 4: 실행 전/후 비용의 실패 테스트와 계산기를 구현한다**
...
- [ ] **Step 2: Server Action schema와 domain service를 강화한다**
...
- [ ] **Step 1: Next.js Server Action/form 문서와 기존 page/action 패턴을 읽는다**
...
- [ ] **Step 3: preview/start Server Actions를 구현한다**
...
- [ ] **Step 4: 작은 Client Component로 panel을 구현한다**
...
- [ ] **Step 5: source-aware Inbox/dashboard를 구현한다**
...
- [ ] **Step 6: 접근성, responsive desktop, 테스트를 검증한다**
...
- [ ] **Step 1: Fixture가 public provider 계약을 구현하되 명확히 표시되게 한다**
...
- [ ] **Step 3: E2E fixture wiring을 완료한다**
...
- [ ] **Step 6: 최종 범위와 secret leakage를 확인한다**
...
- [ ] 외부 호출 전 영상 정보, 총 댓글 수, 예상 quota와 OpenAI 비용을 확인한다.
- [ ] 기본값은 20이고 `20 / 50 / 100 / 1,000`만 허용된다.
- [ ] 저장·분석 댓글은 부모와 답글을 합쳐 선택값과 1,000을 넘지 않는다.
- [ ] 답글 없는 댓글이 누락되지 않고 orphan reply가 없다.
- [ ] 공개 URL job과 결과가 DB에 source provenance와 함께 저장된다.
- [ ] `uncertain`, 낮은 confidence, `needsSecondPass`가 Stage 2로 이동한다.
- [ ] phishing/serious threat는 `risk` 아래로 내려가지 않는다.
- [ ] allowed/context exception만으로 review level이 올라가지 않는다.
- [ ] Stage 1/2 모델과 token/cost snapshot을 구분해 확인할 수 있다.
- [ ] 공개 출처 feedback은 personalization/training에 사용되지 않는다.
- [ ] UI, server/domain, DB가 공개 출처 moderation을 모두 거절한다.
- [ ] Inbox/dashboard에 `공개 URL · 읽기 전용`과 실제 persisted 결과가 표시된다.
- [ ] Fixture는 항상 `TEST FIXTURE`이며 CI에서 실제 외부 API를 호출하지 않는다.
- [ ] 기존 OAuth import/moderation 흐름이 회귀하지 않는다.
- [ ] production에서는 개발 모드가 비활성화된다.
- [ ] unit, DB, evaluation, E2E, lint, build가 모두 통과한다.
